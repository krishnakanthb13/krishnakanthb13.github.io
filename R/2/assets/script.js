document.addEventListener('DOMContentLoaded', () => {
    ...
    let menuStack = [];
    let currentIndex = 0;
    const typingSpeed = 5; // Much faster typing for better feel
    let activeIntervals = [];
    ...
    // Updates JUST the visual highlighting, efficiently
    function updateSelection() {
        const menuItems = content.querySelectorAll('.menu-item');
        menuItems.forEach((item, index) => {
            if (index === currentIndex) {
                item.classList.add('highlight');
                // Ensure cursor is only on the highlighted item
                if (!item.querySelector('.cursor')) {
        ...
        // Auto-scroll to selected item if out of view
    ...
    // Full re-render of the menu (typing effect)
    function displayMenu() {
        clearIntervals();
        content.innerHTML = '';
        const menuItemsHTML = currentMenu.map((item) => `<div class="menu-item">${item.title}</div>`).join('');

        typeText(menuItemsHTML, () => {
            updateSelection(); // Highlight AFTER typing finishes
        });
    }
    ...
    // Simplified Typewriter logic with pre-processing to prevent layout shifts/flashing
    function typeText(htmlContent, callback) {
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = htmlContent;

        // 1. Pre-process: clear text content but preserve structure
        const nodesToType = [];

        function prepareNode(node) {
            if (node.nodeType === Node.TEXT_NODE) {
                const text = node.textContent;
                if (text.trim() !== '') { // Only animate non-empty text
                    node._fullText = text;
                    node.textContent = '';
                    nodesToType.push(node);
                }
            } else if (node.nodeType === Node.ELEMENT_NODE) {
                // Recursively prepare children
                Array.from(node.childNodes).forEach(child => prepareNode(child));
            }
        }

        Array.from(tempDiv.childNodes).forEach(node => prepareNode(node));

        // 2. Append all (now empty) structure to DOM immediately
        // This ensures layout is taken and elements exist for querySelectors
        const nodes = Array.from(tempDiv.childNodes);
        nodes.forEach(node => content.appendChild(node));

        // 3. Animate text filling
        let msgIndex = 0;

        function typeNextNode() {
            if (msgIndex < nodesToType.length) {
                const node = nodesToType[msgIndex];
                const fullText = node._fullText;
                let charIndex = 0;

                const interval = setInterval(() => {
                    if (charIndex < fullText.length) {
                        node.textContent += fullText[charIndex];
                        charIndex++;
                    } else {
    ...
    function navigate(e) {
        // Handle Button Clicks vs Key Presses
        const isUp = (e.key === 'ArrowUp') || (e.target.id === 'upButton' || e.target.closest('#upButton'));
        const isDown = (e.key === 'ArrowDown') || (e.target.id === 'downButton' || e.target.closest('#downButton'));
        const isEnter = (e.key === 'Enter') || (e.key === 'ArrowRight') || (e.target.id === 'enterButton' || e.target.closest('#enterButton'));
        const isBack = (e.key === 'ArrowLeft');

        // Valid Nav check
        if (!isUp && !isDown && !isEnter && !isBack) return;

        const isDetailsView = !!document.querySelector('.details');

        // Prevent default scrolling for arrow keys ONLY if we are in menu mode (not details view)
        // In details view, we want Up/Down to scroll the content naturally
        if ((e.key === 'ArrowUp' || e.key === 'ArrowDown') && !isDetailsView) {
            e.preventDefault();
        }
        // Always prevent default for Left/Right to avoid browser history nav
        if (e.key === 'ArrowLeft') e.preventDefault();

        // If in details view, navigation keys (Up/Down) should just scroll, so we return early
        if (isDetailsView) {
            // Allow Back/Enter to still work to exit details
            if (isBack || isEnter) {
                // Fall through to handle back/enter logic below
            } else {
                return; // Let default scroll happen for Up/Down
            }
        }
        ...
        if (isUp) {
            ...
            playSound(keySound1);
            if (currentIndex < currentMenu.length - 1) {
                currentIndex++;
            } else {
                currentIndex = 0;
            }
            ...
            if (selectedItem.type === 'section') {
                ...
                if (previousState) {
                    currentMenu = previousState.menu;
