...
privacy policy. The extension lives in [`extension/`](extension/); the CLI is
distributed via this repo (GitHub) and doesn't need a store.
...
The store wants a **ZIP of the `extension/` folder's contents** (the
`manifest.json` must be at the ZIP root — do not zip the parent folder).

```bash
cd O/YT/extension
zip -r ../transcript-studio.zip . -x "test/*" ".*"
```

> Exclude `test/` and dotfiles — they aren't needed at runtime. Everything else
> (`manifest.json`, `*.js`, `*.html`, `*.css`, `lib/`, `icons/`) ships.
...
Part of [Transcript Studio for YouTube](README.md) · [MIT licensed](LICENSE).
